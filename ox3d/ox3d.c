...
int board [64];               /* The playing board */
int cplane [384];             /* Cell plane lists */
int linsco [76];              /* Individual line scores */
int lcells [304];             /* Line cell lists */
int clines [448];             /* Cell line lists */
int clinh [448];              /* Cell line histograms */
...
void intro ()
{
   ...
   printf ("Restart  - aborts the current game and starts a new one \n");
   printf ("Quit     - aborts the game\n");
   printf ("Display  - forces a complete re-display of the board\n");
   printf ("Suggest  - asks for a suggestion of the best move.\n");
   printf ("Atomatic - causes the program to take over the current player\n");
...
char *bckup;
{
   ...
   bck += sizeof (plnsco);
   memcpy (bck, (char *) &player, sizeof (player));
   bck += sizeof (player);
   memcpy (bck, (char *) &sign, sizeof (sign));
   bck += sizeof (sign);
   memcpy (bck, (char *) &winner, sizeof (winner));
   bck += sizeof (winner);
   memcpy (bck, (char *) win, sizeof (win));
   bck += sizeof (win);
   memcpy (bck, (char *) kill, sizeof (kill));
   bck += sizeof (kill);
   memcpy (bck, (char *) &move, sizeof (move));
   bck += sizeof (move);
   memcpy (bck, (char *) &movcnt, sizeof (movcnt));
   bck += sizeof (movcnt);
   memcpy (bck, (char *) &strcnt, sizeof (strcnt));
   bck += sizeof (strcnt);
   memcpy (bck, (char *) &lincnt, sizeof (lincnt));
   bck += sizeof (lincnt);
...
char *bckup;
{
   ...
   bck += sizeof (plnsco);
   memcpy ((char *) &player, bck, sizeof (player));
   bck += sizeof (player);
   memcpy ((char *) &sign, bck, sizeof (sign));
   bck += sizeof (sign);
   memcpy ((char *) &winner, bck, sizeof (winner));
   bck += sizeof (winner);
   memcpy ((char *) win, bck, sizeof (win));
   bck += sizeof (win);
   memcpy ((char *) kill, bck, sizeof (kill));
   bck += sizeof (kill);
   memcpy ((char *) &move, bck, sizeof (move));
   bck += sizeof (move);
   memcpy ((char *) &movcnt, bck, sizeof (movcnt));
   bck += sizeof (movcnt);
   memcpy ((char *) &strcnt, bck, sizeof (strcnt));
   bck += sizeof (strcnt);
   memcpy ((char *) &lincnt, bck, sizeof (lincnt));
   bck += sizeof (lincnt);
