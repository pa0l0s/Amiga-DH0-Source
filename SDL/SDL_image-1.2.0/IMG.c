...
SDL_Surface *IMG_Load(const char *file)
{
	...
	if (src == NULL)	{
		fprintf(stderr, "error SDL_RWFromFile %s: %s\n", file, SDL_GetError());
	}
...
SDL_Surface *IMG_LoadTyped_RW(SDL_RWops *src, int freesrc, char *type)
{
	...
	if ( src == NULL ) {
		fprintf(stderr, "error IMG_LoadTyped_RW %s: %s\n", type, "src == NULL nothing to do!");
		return(NULL);
	}
...
//#endif
	...
	if (image == NULL){
		fprintf(stderr, "IMGLIB: image == NULL! \n");
	}
	return(image);
}
...
SDL_Surface *IMG_LoadPNG(const char *file)
{
    SDL_RWops *src = SDL_RWFromFile(file, "rb");
	char *ext = strrchr(file, '.');
