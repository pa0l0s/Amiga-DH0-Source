...
//============================================================================
int initDefaultWindow(void)
{
	return initWindow(320,200);
}

int initWindow(int winWidth, int winHeight)
{
	g_nWinWidth = winWidth;
	g_nWinHeight = winHeight;
...
void printText(char text[], int x, int y, int pen)
{
	/*ustawia kolor*/
	SetAPen(m_pWin->RPort,pen);
	/*ustaw pozycje x,y*/
	Move(m_pWin->RPort,x,y);
	/*wypisz tekst*/
	Text(m_pWin->RPort,text,strlen(text));
}

int getTextXCenter(char text[])
{
	int x=g_nWinWidth/2-(strlen(text)*4);
	return x;
}
//============================================================================
static void setWindowTag(Tag name, Tag value)
...
{
	Draw(m_pWin->RPort,x,y);
}

void windowClear(void)
{
		   /*ustawia na kolor tla*/
   SetAPen(m_pWin->RPort,0);
   /*wymazuje ekran t³em*/
   RectFill(m_pWin->RPort,0,0,g_nWinWidth,g_nWinHeight);
}
